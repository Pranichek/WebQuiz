...
export async function mentorSettings() {
    ...
    socket.on("data_question_mentor", data => {
        ...
        let timeFlag = localStorage.getItem("flag_time");
        let questionTime = parseInt(data.time_question); 

        if (timeFlag == "false") {
            timerDisplay.textContent = "00:00";
            localStorage.setItem("time_flag", 0);
            localStorage.setItem("end_time", Date.now() + questionTime * 1000);
        } else {
