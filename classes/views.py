...
def render_data_class():
    ...
    class_id = flask.request.args.get("class_id")
    class_item = Classes.query.get(class_id)
    user = User.query.get(class_item.user_id)
    email = user.email
    users_list = []
    ...
    term_task = class_item.term_task

    start_time = start_time.split("@")
    term_task = term_task.split("@")
    ...
    for elem in range(len(data_topic[:-1])):
        time_upload = start_time[elem].replace("/", ".")

        path_mentor_files = os.path.abspath(
            join(__file__, "..", "..", "userprofile", "static", "images", "edit_avatar", f"{email}", "classes", f"{time_upload}")
        )

        if os.path.isdir(path_mentor_files):
            files = os.listdir(path_mentor_files)
            for image_name in files:
                img_url = flask.url_for(
                    "profile.static",
                    filename=f"images/edit_avatar/{email}/classes/{time_upload}/{image_name}"
                )
                img_task.append(img_url)
